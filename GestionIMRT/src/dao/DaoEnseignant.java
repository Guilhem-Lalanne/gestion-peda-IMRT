...
public class DaoEnseignant {
    ...
    public void getEnseignant(List<Enseignant> enseignant) throws SQLException {
        ...
        while (rset.next()) {       // traitement du résulat
            ...
            int idEmployeur = 1;
            int docAdmPhotoCarteNatInt = rset.getInt(12);
            int docAdmPhotoCarteSecuInt = rset.getInt(13);
            int docAdmJustTravInt = rset.getInt(12);
            int docAdmRecepArreteNominInt = rset.getInt(12);
            String userLogin = rset.getString(1);
            String userNom = rset.getString(2);
            String userGroupe = rset.getString(3);
            tools.debug(" docAdmPhotoCarteNatInt " + docAdmPhotoCarteNatInt);
            //conversion pour passage en boolean car non gere par oracle
            ...
            if (docAdmPhotoCarteSecuInt == 0) {
                docAdmPhotoCarteSecu = false;
            }
            if (docAdmJustTravInt == 0) {
                docAdmJustTrav = false;
            }
             if (docAdmRecepArreteNominInt == 0) {
                docAdmRecepArreteNomin = false;
            }
            
            tools.debug(" boolean docAdmPhotoCarteNat=true;  " + docAdmPhotoCarteNat);
